...
public class BookServiceImpl implements BookService {
     ...
     */
    @Override
    public Result<List<BookVO>> queryViewer(BookQueryDto bookQueryDto) {
        List<BookVO> noticeList = bookMapper.query(bookQueryDto);
        Integer totalCount = bookMapper.queryCount(bookQueryDto);
        return PageResult.success(noticeList, totalCount);
    }
    ...
    public Result<List<ChartVO>> daysQuery(Integer day) {
        ...
        // 由于数据量太少，这里直接让数据随机增加，从而更好展现数据，不想要这种效果，注释掉即可
        for (ChartVO chartVO : chartVOS) {
            chartVO.setCount(chartVO.getCount() * new Random().nextInt(10000));
        }
        return ApiResult.success(chartVOS);
    }

    @Override
    public Result<List<ChartVO>> bookType() {
        return ApiResult.success(bookMapper.bookType());
    }
}
